...
import { getOrCreateUserByPhone, getOrCreateUserByGoogle, findUserById, updateUserProfile } from "../services/user.js";
...
authRouter.post("/phone/send-code", sendCodeLimiter, async (req, res) => {
  ...
  try {
    const extra = await sendCode(parsed.data.phone);
...
authRouter.patch("/me", requireAuth, async (req, res) => {
  ...
  const updated = await updateUserProfile(uid, { name: parsed.data.name, avatarUrl: parsed.data.avatarUrl });
  if (!updated) {
    res.status(404).json({ error: "Пользователь не найден" });
    return;
  }
